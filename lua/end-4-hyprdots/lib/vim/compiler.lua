local path_sep = require("end-4-hyprdots").path_sep
...
function M.compile(flavour)
	local theme = require("end-4-hyprdots.lib.mapper").apply(flavour)
	local lines = {
		string.format(
			[=[
...
set termguicolors
set background=%s
let g:colors_name = "end-4-hyprdots-%s"]=],
			(flavour == "light" and "light" or "dark"),
			flavour
		),
	}
	...
	if vim.g.end_4_hyprdots_debug then -- Debugging purpose
	...
	if not f then
		local err_path = (path_sep == "/" and "/tmp" or os.getenv "TMP") .. "/end-4-hyprdots_error.lua"
		print(string.format(
			[[end-4-hyprdots (error): Most likely some mistake made in your end-4-hyprdots config
