...
function M.apply(flavour)
	flavour = flavour or require("end-4-hyprdots").flavour

	local _O, _C, _U = O, C, U -- Borrowing global var (setfenv doesn't work with require)
	O = require("end-4-hyprdots").options
	C = require("end-4-hyprdots.palettes").get_palette(flavour)
	U = require "end-4-hyprdots.utils.colors"
	...
	C.dim = O.dim_inactive.shade == "dark"
			and U.vary_color(
				{ light = U.darken(C.base, dim_percentage, C.mantle) },
				U.darken(C.base, dim_percentage, C.mantle)
			)
		or U.vary_color(
			{ light = U.lighten("#FBFCFD", dim_percentage, C.base) },
			U.lighten(C.surface0, dim_percentage, C.base)
		)

	local theme = {}
	theme.syntax = require("end-4-hyprdots.groups.syntax").get()
	theme.editor = require("end-4-hyprdots.groups.editor").get()
	local final_integrations = {}

	-- https://github.com/end-4-hyprdots/nvim/pull/624
	...
	for integration in pairs(O.integrations) do
		...
		if type(O.integrations[integration]) == "table" then
			...
			if O.integrations[integration] == true then
				local default = require("end-4-hyprdots").default_options.integrations[integration]
		...
		if cot then
			final_integrations = vim.tbl_deep_extend(
				"force",
				final_integrations,
				require("end-4-hyprdots.groups.integrations." .. integration).get()
			)
		end
	end

	theme.integrations = final_integrations -- plugins
	theme.terminal = require("end-4-hyprdots.groups.terminal").get() -- terminal colors
