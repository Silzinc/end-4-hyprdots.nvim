---@class End4Hyprdots
---@field options End4HyprdotsOptions
---@field setup fun(opts: End4HyprdotsOptions?)

---@alias CtpFlavor "dark" | "light" | "auto"
...
---@class CtpFlavors<T>: {all: T, dark: T, light: T }
...
---@class End4HyprdotsOptions
-- Changes the flavor based on the background. See `:h background` for more info.
---@field background CtpBackground?
-- By default end-4-hyprdots writes the compiled results into the system's cache directory.
...
-- End4Hyprdots highlights can be overwritten here.
...
---@class CtpBackground
-- End4Hyprdots flavor to use when `:set background=dark` is set.
---@field dark CtpFlavor?
-- End4Hyprdots flavor to use when `:set background=light` is set.
...
--    ui = {
--        kind = require("end-4-hyprdots.groups.integrations.lsp_saga").custom_kind(),
--    },
-- }
-- ```
...
---@field enabled boolean
-- Set to a End4Hyprdots color name to use for the split separator.
