...
local function get_flavour(default)
	local flavour
	if default and default == M.flavour and vim.o.background ~= (M.flavour == "light" and "light" or "dark") then
	...
	if flavour and not M.flavours[flavour] then
		vim.notify(
			string.format(
				"end-4-hyprdots (error): Invalid flavour '%s', flavour must be 'light', 'dark' or 'auto'",
				flavour
			),
	...
	return flavour or M.options.flavour or M.options.background[vim.o.background]
end
...
---@type fun(user_conf: End4HyprdotsOptions?)
...
vim.api.nvim_create_user_command(
	"End4Hyprdots",
	function(inp) vim.api.nvim_command("colorscheme end-4-hyprdots-" .. get_flavour(inp.args)) end,
	{
		nargs = 1,
...
vim.api.nvim_create_user_command("End4HyprdotsCompile", function()
	for name, _ in pairs(package.loaded) do
		if name:match "^end%-4%-hyprdots." then package.loaded[name] = nil end
	end
	M.compile()
	vim.notify("End4Hyprdots (info): compiled cache!", vim.log.levels.INFO)
	vim.cmd.colorscheme "end-4-hyprdots"
end, {})

if vim.g.end_4_hyprdots_debug then
	vim.api.nvim_create_autocmd("BufWritePost", {
		pattern = "*/end-4-hyprdots/*",
		callback = function()
			vim.schedule(function() vim.cmd "End4HyprdotsCompile" end)
		end,
	})
end
